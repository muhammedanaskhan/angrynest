...
const Page = async ({ params }: { params: { id: string } }) => {
    ...
    return(
        <section>
            <ProfileHeader
                accountId={userInfo._id} // person whose profile is being viewed
                    ...
                    <TabsList className='tab'>
                        {profileTabs.map((tab) => (
                            <TabsTrigger key={tab.label} value={tab.value} className='tab_option'>
                                ...
                                {tab.label === 'Threads' && <p className="ml-1 rounded-full bg-black px-2 py-1 !text-tiny-medium text-light-1">    
